...
一套自用、长期维护的 Loon 规则配置，目标是把「国内直连优先、国外按需代理、各类 App 去广告」这套思路做到 **可读、可复用、可自查**——不是黑箱订阅，每条规则为什么这么写都有注释。

- **平台**：Loon（iOS / iPadOS）
- **不含任何可用节点 / 机场订阅**：纯规则 + 插件框架，节点需自填（见「节点配置放哪里」）
...
仅供个人学习与网络优化研究。节点/订阅/破解类插件（如 YouTube Music 解锁、后台播放）请自行评估合规与封号风险，与本仓库无关。
